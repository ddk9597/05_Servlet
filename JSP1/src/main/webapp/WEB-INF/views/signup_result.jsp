...
1) JSP에 작성된 Java 코드 주석 처리 시 사용
2) JSP전용 주석은 컴파일 단계에서 걸러지기 때문에 응답 화면(브라우저)에서 볼 수 없음
...
<body>
	...
	<%-- 
		requset 변수가 전달받은 HttpServletRequest 객체를 참조하는 변수
		
		<% %>  태그: Scriptlet(스크립틀릿) JSP에서 자바 코드를 작성할 수 있게 하는 태그
					 <% Java 코드 %> 형식으로 작성
					 
		<%= %> 태그: Expression(표현식)
					 JSP에서 Java의 값을 HTML 화면에 출력(표현)하는 태그
					 (Java 값을 html 코드로 변환)

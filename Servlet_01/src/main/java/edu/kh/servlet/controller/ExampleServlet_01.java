...
import jakarta.servlet.http.HttpServlet; // TomCat이 제공하는 class
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;

public class ExampleServlet_01 extends HttpServlet{

	// get 방식 요청(html에서 method : get 인 form 태그 제출) 처리하는 메서드
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

		/* * 매개 변수 req, resp
		 * 
		 * HttpServletRequest : 클라이언트 요청 시 자동 생성되는 객체
		 *  - 요청 시 전달된 데이터,
		 *    요청 한 클라이언트의 정보,
		 *    요청을 유연하게 처리하기 위한 객체 등을 제공
		 * 
		 * HttpServletResponse : 클라이언트 요청 시 자동 생성되는 객체
		...
		// 제출 된 이름, 나이 == 전달 인자(Parameter) == 전달 받은 값
		...
		// <input age ="inputAge">에 작성되어 제출된 값 얻어오기
		 ...
		 * Html 코드를 출력      -> 출력 받은 HTML 코드를 해석해서 화면 구성
		 * 
		...
		sb.append( String.format("<h1> %s님의 나이는 %s세 입니다 </h1>", inputName, inputAge) );
